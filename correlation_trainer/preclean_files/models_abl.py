...
class GIN_Model(nn.Module):
    def __init__(
        ...
        self.dinp = 2
        self.dual_gcn = dual_gcn
        self.unroll_fgcn = False
        self.num_zcps = num_zcps
        self.randopupdate = False
        self.op_embedding_dim = op_embedding_dim
        self.back_opemb_only = False
        self.detach_mode = "default"
        self.node_embedding_dim = node_embedding_dim
        self.zcp_embedding_dim = zcp_embedding_dim
        self.hid_dim = hid_dim
        self.gcn_out_dims = gcn_out_dims
        ...
        self.fb_conversion_dims = fb_conversion_dims
        self.back_opemb = True
        self.opemb_direct = False
        self.back_y_info = False
        self.backward_gcn_out_dims = backward_gcn_out_dims
        self.ensemble_fuse_method = "add"
        self.updateopemb_dims = updateopemb_dims
        self.updateopemb_scale = updateopemb_scale
        self.mlp_dims = mlp_dims
        self.nn_emb_dims = nn_emb_dims
        ...
        self.none_op_ind = none_op_ind
        self.gtype = gtype
        self.back_dense = True
        self.residual = True
        self.unique_attn_proj = False
        self.opattention = True
        self.leakyrelu = True
        self.attention_rescale = False
        self.replace_bgcn_mlp_dims = replace_bgcn_mlp_dims
        self.back_mlp = False
        if not self.opattention:
            self.op_embedding_dim = self.gcn_out_dims[0]
                ...
                if self.back_mlp:
                    ...
                    if self.back_opemb_only:
                        ...
                        if self.back_opemb and self.back_y_info:
                            ...
                            in_embedding = b_y
            elif self.detach_mode=='detach_all':
                if self.back_mlp:
                    if self.back_opemb_only:
                        in_embedding = op_emb
                    else:
                        if self.back_opemb and self.back_y_info:
                            in_embedding = torch.cat(
                                (
                                    op_emb.detach(),
                                    y.detach(),
                                    b_y.unsqueeze(dim=1).repeat(1, y.shape[1], 1).detach()
                                ),
                                dim = -1)
                        elif self.back_opemb==False and self.back_y_info:
                            in_embedding = torch.cat(
                            ...
                            in_embedding = b_y.unsqueeze(dim=1).repeat(1, y.shape[1], 1).detach()
                else:
                    if self.back_opemb_only:
                        in_embedding = op_emb
                    else:
                        if self.back_opemb and self.back_y_info:
                            ...
                            in_embedding = b_y.detach()
            elif self.detach_mode=='detach_none':
                if self.back_mlp:
                    if self.back_opemb_only:
                        in_embedding = op_emb
                    else:
    ...
    def forward(self, x_ops_1=None, x_adj_1=None, x_ops_2=None, x_adj_2=None, zcp=None, norm_w_d=None):
        ...
        if self.dual_gcn:
            ...
            for tst in range(self.num_time_steps):
                if self.unroll_fgcn:
                    y_2 = self._forward_pass(y_2, adjs_2, op_emb_2)
                else:
