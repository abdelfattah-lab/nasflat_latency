...
class FewShotPredictor(nn.Module):
    def __init__(self,                          
        ...
        for i in range(feat_depth):
            if i==0:
                n = nn_emb_dim + hw_emb_dim + zcp_dims
            else:
                n = feat_layer_size
            if i==feat_depth-1:
                m = 1
    ...
    def forward(self, arch, device_input, zcp=None, device_offset=None):
        ############### EmbTable ###############
        # (batch_size, hw_emb_dim)
        device_emb = self.dev_emb(device_input).squeeze(1)
        # device_emb = self.emb_mlp_net(device_emb)
        if self.search_space=='nasbench201':
            ...
            gcn_out = self.relu(self.fc3(gcn_out))
        ############### Feature Interaction ###############
        out = self.sigmoid(self.feat_int_net(torch.cat([gcn_out, device_emb, zcp], dim=1)))
        return out
