...
def objective(trial):
    ...
    with open(f'optuna_run_fbnet/{trial.number}.txt', 'r') as f:
...
with open('optuna_fbn.log', 'a') as log_file:
    ...
    for trial in sorted_trials[:5]:
        log_file.write(f"Trial {trial.number}, Params: {trial.params}, Value: {trial.value}\n")
# # if __name__ == "__main__":
# study = optuna.create_study(direction='maximize', study_name='optuna_fbn')
# study.optimize(objective, n_trials=300)

# # Saving the results to optuna_nb2.log
# with open('optuna_fbn.log', 'w') as log_file:
