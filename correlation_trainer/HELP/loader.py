...
class Data:
    def __init__(self, mode,
        ...
        self.idx_emb = False
        self.index_mapper = {x: idx for idx, x in enumerate(list(set(self.meta_train_devices + self.meta_valid_devices + self.meta_test_devices)))}
        print(self.index_mapper)
        ...
        for device in meta_train_devices + meta_valid_devices + meta_test_devices:
            ...
            valid_idx = torch.arange(len(self.archs))
            ...
            if self.remove_outlier:
                ...
                self.train_idx[device] = train_idx[
                    np.argsort(self.latency[device][train_idx])[
                        int(len(train_idx)*0):int(len(train_idx)*1)]]
                self.valid_idx[device] = valid_idx[
                    np.argsort(self.latency[device][valid_idx])[
                        int(len(valid_idx)*0):int(len(valid_idx)*1)]]
        ...
        # self.hw_emb_idx = torch.load('/home/ya255/projects/unified_nas_representation/HELP/data/nasbench201/hardware_embedding_tensors/hardware_embedding_index.pt')
    ...
    def generate_episode(self):
        ...
        for t in rand_device_idx:
            ...
            if self.idx_emb:
                v = [int(b) for b in [*bin(self.index_mapper[device])[2:]]]
                v = [0]*(hw_embed.shape[0] - len(v)) + v
                hw_embed = torch.FloatTensor(v)
    ...
    def generate_test_tasks(self, split=None):
        ...
        for device in device_list:
            task_temp = self.get_task(device)
            # print(str(device), " finetune len : ", len(task_temp[1][0]))
            # print(str(device), " qry len      : ", len(task_temp[3][0]))
