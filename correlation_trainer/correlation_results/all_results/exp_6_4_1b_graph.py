...
def generate_graphs_from_csv(file_path,space):
    ...
    df = df[["task_index", "transfer_sample_size", "spr", "sampling_metric"]]
    ...
    for task_ix in task_indices:
        ...
        for metric in sampling_metrics:
            ...
            plt.plot(df_metric['transfer_sample_size'], df_metric['spr'], marker="o", color=color_map[metric], label=metric)
        
        plt.title(f'Task {task_ix}')
        plt.xlabel('Number Of Samples From Target Device')
        plt.ylabel('SPR')
        plt.grid(True)
        plt.legend()
...
def generate_combined_graph_from_csv(file_path, space):
    ...
    # Filter out the required columns
    df = df[["task_index", "transfer_sample_size", "spr", "sampling_metric", "spr_std"]]
    ...
    for idx, task_ix in enumerate(task_indices):
        ...
        for metric in sampling_metrics:
            ...
            y = df_metric['spr']
            x = df_metric['transfer_sample_size']
            ax.plot(df_metric['transfer_sample_size'], df_metric['spr'], linewidth=2, marker="o", color=color_map[metric], label=metric_map[metric])
        ...
        ax.set_xlabel('Number Of Samples From Target Device', fontsize=20)
...
def generate_2s_combined_graph_from_csv(file_path_fbnet, file_path_nb201):
    ...
    # Filter out the required columns
    df_fbnet = df_fbnet[["task_index", "transfer_sample_size", "spr", "sampling_metric", "spr_std"]]
    ...
    df_nb201 = df_nb201[["task_index", "transfer_sample_size", "spr", "sampling_metric", "spr_std"]]
    ...
    for idx, task_ix in enumerate(task_indices):
        ...
        # ax.set_title(f'Task {task_ix}', fontsize=20)
        ax.text(0.5,.05,f"Task {task_ix}", fontsize=20,
    ...
    # Create a shared legend for all subplots
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.11), ncol=len(sampling_metrics), fontsize=20)

    fig.text(0.5, -0.01, 'Number Of Samples From Target Device', ha='center', va='center', fontsize=20)
    plt.tight_layout()

    # Instead of the original file_path.split("/")[0] use a generic name for directory_path
    directory_path = 'graphs/aggr_study_6_4_1b/combined_graphs/'
    if not os.path.exists(directory_path):
        os.makedirs(directory_path)
...
def generate_geomean_plot(file_path_fbnet, file_path_nb201):
    # Read the CSVs
    df_fbnet = pd.read_csv(file_path_fbnet)
    df_nb201 = pd.read_csv(file_path_nb201)
    ...
    # Get the first two unique task indices
    task_indices_fbnet = sorted(list(set(df_fbnet.task_index.tolist())))[:4]
    task_indices_nb201 = sorted(list(set(df_nb201.task_index.tolist())))[:4]

    # Filter data based on these indices
    # df_fbnet = df_fbnet[df_fbnet["task_index"].isin(task_indices_fbnet)]
    # df_nb201 = df_nb201[df_nb201["task_index"].isin(task_indices_nb201)]
    df = df_nb201[df_nb201["task_index"].isin(task_indices_nb201)]
    # df = df_fbnet[df_fbnet["task_index"].isin(task_indices_fbnet)]

    # Concatenate the DataFrames
    # df = pd.concat([df_fbnet, df_nb201])
    # Calculate the geometric mean for the specified metrics
    geomean_metrics = ["zcp", "a2vcatezcp", "arch2vec", "cate"]
    task_indices = sorted(list(set(df.task_index.tolist())))
    for task_ix in task_indices:
        for sample_size in df['transfer_sample_size'].unique():
            spr_values = df[(df['task_index'] == task_ix) & 
                            (df['transfer_sample_size'] == sample_size) &
                            (df['sampling_metric'].isin(geomean_metrics))]['spr'].values
            spr_std_values = df[(df['task_index'] == task_ix) & 
                                (df['transfer_sample_size'] == sample_size) &
                                (df['sampling_metric'].isin(geomean_metrics))]['spr_std'].values

            geomean_spr = np.exp(np.mean(np.log(spr_values)))
            geomean_spr_std = np.exp(np.mean(np.log(spr_std_values)))

            df = df.append({'task_index': task_ix, 
                            'transfer_sample_size': sample_size, 
                            'spr': geomean_spr, 
                            'sampling_metric': 'geomean (Ours)', 
                            'spr_std': geomean_spr_std}, ignore_index=True)

    # Define the metrics to be used for plotting
    metrics = ["random", "params", "geomean (Ours)"]
    ...
    metric_map = {
            "random": "Random",
            "params": "Params",
            "geomean (Ours)": "GeoMean (Arch2Vec, CATE, ZCP, CAZ)"
    }
    # Get unique sampling metrics
    # only select metrics from metrics
    df = df[df["sampling_metric"].isin(metrics)]
    ...
    for idx, task_ix in enumerate(task_indices):
        ...
        # ax.set_title(f'Task {task_ix}', fontsize=20)
        ax.text(0.5,.9,f"Task {task_ix}", fontsize=20,
            horizontalalignment='center', bbox=dict(facecolor='white', edgecolor='gray', boxstyle='round,pad=0.3'),
            transform=ax.transAxes)
        # Set x axis range 
        # ax.set_xlim([0, 200])
        ...
        # Ensure that tick params are only 1 decimal precision for ONLY y axis
        ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.1f'))
        # set x axis numbers to be displayed as integers
        ax.set_xscale('log', basex=2)
        ax.xaxis.set_major_formatter(plt.FormatStrFormatter('%1.f'))
        ...
        # if task_ix.__contains__("N1"):
        #     ax.set_ylim([0.65, 0.9])
        # if task_ix.__contains__("N2"):
        #     ax.set_ylim([0.55, 0.7])
        # if task_ix.__contains__("N3"):
        #     ax.set_ylim([0.65, 0.9])
        # if task_ix.__contains__("N4"):
        #     ax.set_ylim([0.82, 0.95])
        # set y axis log
        # ax.set_yscale('log')
        # ax.set_xscale('log')
        # x axis log with base 2
        # ax.set_xlim([50, 500])
    ...
    # Create a shared legend for all subplots
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.20), ncol=len(sampling_metrics), fontsize=20)
    ...
    # Save the figure
    plt.savefig(f"{directory_path}/gm_combined_tasks.png", bbox_inches='tight')
    plt.savefig(f"{directory_path}/gm_combined_tasks.pdf", bbox_inches='tight')
    plt.close()


# # Generate graphs for the provided CSV
# generate_graphs_from_csv('aggr_study_6_4_1b/fbnet_samp_eff.csv', space='fbnet')
# generate_graphs_from_csv('aggr_study_6_4_1b/nb201_samp_eff.csv', space='nb201')
# generate_combined_graph_from_csv('aggr_study_6_4_1b/fbnet_samp_eff.csv', space='fbnet')
# generate_combined_graph_from_csv('aggr_study_6_4_1b/nb201_samp_eff.csv', space='nb201')


# generate_2s_combined_graph_from_csv('aggr_study_6_4_1b/fbnet_samp_eff.csv', 'aggr_study_6_4_1b/nb201_samp_eff.csv')
generate_geomean_plot('aggr_study_6_4_1b/fbnet_samp_eff.csv', 'aggr_study_6_4_1b/nb201_samp_eff.csv')