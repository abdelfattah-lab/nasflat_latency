...
def generate_latex_table_from_csv(file_path):
    ...
    df = df.pivot_table(index="Task", 
                             ...
                             values="SPR", 
                             aggfunc="first")
    # # Display the Sampler column in a certain order
    # order_sampler = ['random', 'latency', 'params', 'arch2vec', 'cate', 'zcp', 'a2vcatezcp']
    # df = df[order_sampler]
    # tasks_ = [1,2,3,4]
    # # only display Task tasks_
    # df = df.loc[tasks_]
    # # Add a "\bR" to the highest spr in each column
    # df = df.apply(lambda col: col.apply(lambda spr: f"\\bR{spr}" if spr == col.max() else spr), axis=0)
...
# For nb201_samp_eff.csv
latex_nb201 = generate_latex_table_from_csv('aggr_study_6_3_1_sampler/nb201_samp_eff.csv')
print(latex_nb201)

# For fbnet_samp_eff.csv
latex_fbnet = generate_latex_table_from_csv('aggr_study_6_3_1_sampler/fbnet_samp_eff.csv')
print(latex_fbnet)
