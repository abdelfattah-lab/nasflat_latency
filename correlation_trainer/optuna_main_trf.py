...
parser.add_argument('--optuna_run_id', type=int, default=0)
...
sample_size = args.sample_sizes[0]
transfer_sample_size = args.transfer_sample_sizes[0]
avg_spr = []
...
# Save the avg_spr into a file called "optuna_run/{}.txt".format(args.optuna_run_id) Create optuna_run directory if it doesnt exist
if not os.path.exists('optuna_run_{}'.format(args.space)):
    os.makedirs('optuna_run_{}'.format(args.space))
with open('optuna_run_{}/{}.txt'.format(args.space, args.optuna_run_id), 'w') as f:
    f.write(str(avg_spr))
