...
if True:
    ...
    spaces = ['ENAS', 'nb201']
    configurations = list(itertools.product(timesteps, back_mlp, back_opemb, back_y_info, spaces))
    base_command = (
        'True,archabl,True,30000,4,python main_abl.py --seed 42 --name_desc archabl '
    ...
    for config in configurations:
        timesteps, back_mlp, back_opemb, back_y_info, space = config
        command = base_command.format(space=space, timesteps=timesteps)
        if back_mlp: 
            command += ' --back_mlp'
        if back_opemb: 
